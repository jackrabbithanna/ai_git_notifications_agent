...
| pi (pi.dev) | "Chat with my inbox" and "deep-dive a thread" — **M6, done 2026-09-20**: `pi --mode rpc` sidecar (`@earendil-works/pi-coding-agent` 0.86.1, BYO binary or the repo's `agent/node_modules`), app-owned agent dir under `$XDG_DATA_HOME/gitinbox/agent`, only GitInbox's extension tools (no built-in tools), Ollama via `models.json`; tools call a loopback JSON API (`internal/localapi`) with a per-process bearer token; forge access is read-only through the app's own MCP client / REST source, never a second server process. |
...
- pi: headless RPC mode (`pi --mode rpc`, JSONL over stdio) intended for embedding; Ollama via OpenAI-compatible `models.json` provider; TS extensions add tools. **Verified in M6 (0.86.1):** `PI_CODING_AGENT_DIR` replaces `~/.pi/agent`; `--no-builtin-tools --no-extensions -e <file>` leaves only our tools; extensions load via jiti and can `import { Type } from "typebox"`; `--no-approve --offline` keep it quiet and non-interactive. pi has no native MCP client (extensions/skills instead), so GitHub access goes through the app's read-only MCP client behind the local API rather than a second server.
...
| `localapi` | Loopback HTTP JSON API (127.0.0.1, random port, per-process bearer token) over threads (top/search/detail/find), summaries, live comments, Mine, impact analyses, PR changes, read-only GitHub tool passthrough, local actions and drafts; the pi extension's tools call it (M6). | `net/http` |
| `agent` | pi sidecar: locate the binary, prepare the agent dir (`models.json`, `settings.json`, embedded `ext/gitinbox.ts`, `prompt.md`), spawn `pi --mode rpc`, JSONL client, transcript + UI events (M6). | `os/exec` |
| `services` | Wails services: `InboxService`, `MineService`, `ImpactService`, `ProfilesService`, `SettingsService`, `DiagnosticsService`, `AgentService` (M6). | Wails v3 |
...
## 6. pi integration (M6 — built)
- `internal/agent.Manager` spawns `pi --mode rpc --no-builtin-tools --no-extensions -e <agentdir>/extensions/gitinbox.ts --no-skills --no-prompt-templates --no-themes --no-context-files --no-approve --offline --session-dir … --provider ollama --model <id> --thinking off --system-prompt <rendered prompt.md>` with `PI_CODING_AGENT_DIR=$XDG_DATA_HOME/gitinbox/agent`, `GITINBOX_API_URL`/`GITINBOX_API_TOKEN` in the environment; reads JSONL events, keeps the transcript (user / assistant / tool / error entries) and forwards compact `agent:event`s (`text_delta`, `tool_start`, `tool_end`, `agent_end`, …) to the UI; auto-cancels any `extension_ui_request` dialog. BYO pi: override path → `pi` on PATH → `agent/node_modules/.bin/pi` (the repo pins `@earendil-works/pi-coding-agent` in `agent/package.json` for development).
- Isolation: `Prepare` writes `models.json` (provider `ollama`, `<ollama.url>/v1`, `openai-completions`, `compat{supportsDeveloperRole:false, supportsReasoningEffort:false}`, every model the Ollama server lists), `settings.json` (`defaultProjectTrust: never`, telemetry off, compaction on) and the embedded extension; no built-in tools, so pi has no shell or file access at all.
- Extension `ext/gitinbox.ts` (typebox schemas, `fetch` to the local API, 40 KB output cap): `list_accounts`, `list_top_priority`, `search_threads`, `get_thread`, `find_thread`, `get_thread_summary`, `get_thread_comments` (live via the forge: issue comments, PR reviews + review comments, GitLab notes), `list_mine`, `list_impact`, `get_pr_analysis` (runs the analysis / note when asked), `get_pr_changes`, `mark_done`, `mark_read`, `undo_done`, `snooze`, `draft_reply` (stored in `drafts`, migration 0007, shown in the inbox Details panel; never posted), `github_read` (raw pass-through restricted to `ghmcp.AllowedTools(readonly)` regardless of the account's write mode).
- Deviation from the original plan: pi has no MCP client and a second `github-mcp-server` per account would duplicate token handling, so deep-dives go through the app's existing read-only client behind `localapi` instead of attaching the server to pi. Same guarantee (reads only), one process per account.
- UI: **Agent** view (streamed transcript, collapsible tool calls, model select, Abort / New session / Start / Stop, suggestion chips), **Deep-dive** button on every inbox row (pre-fills and sends a deep-dive prompt), Settings → Agent (enable, autostart, pi path, model, thinking), Diagnostics → Agent card; CLI `agent status|chat|models` and `settings set agent.*`.
...
ai_github_notifications_agent/
  ...
  cmd/gitinbox/              # headless CLI: mcp tools|call, sync, mine, analyze <repo#pr>, judge, summarize, eval, agent
  internal/{ghmcp,mcpbin,store,classify,filter,profiles,judge,llm,pipeline,scoring,secrets,localapi,agent,services}/
  internal/judge/{jev,ollama}/  internal/llm/ollama/  internal/store/migrations/*.sql
  internal/agent/{ext/gitinbox.ts,prompt.md}   # embedded pi extension + system prompt
  internal/profiles/builtin/{generic.yaml,civicrm.yaml}
...
- **M6 – pi sidecar — done 2026-09-20; verified live.** pi 0.86.1 from `agent/node_modules` with `qwen3.5:9b` over Ollama's OpenAI endpoint: sidecar up in 1.8 s; "what needs me most" → one `list_top_priority` call, correct three-item answer with links in 16 s; a deep-dive of civicrm-core#36923 → `get_thread` + `get_thread_comments` (GitHub MCP read tools) + `get_pr_analysis`, grounded answer (asked to test the demo site, state table, next action) in 29 s; `draft_reply` stored a draft in `drafts`. Pre-coding experiment settled the three open questions (agent-dir env var, tool restriction flags, typebox import resolution via jiti). `internal/localapi` (16 routes, bearer token, tests), `internal/agent` (locate, dir, rpc, manager; tests with a fake pi), `source.Discusser` for both forges, migration 0007 (`drafts`), `AgentService`, Agent view + deep-dive button + draft panel + settings/diagnostics cards, CLI `agent`. Original scope: `AgentService`, `pi-gh-inbox` extension, chat panel + deep-dive drawer.
- **Later:** macOS/Windows CI, OAuth device flow, low-risk writes (reactions/labels), optional cloud LLM, bundling a pi runtime, agent-driven summaries ("summarise these five"), a markdown renderer for agent replies.
