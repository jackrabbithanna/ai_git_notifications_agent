...
| Output | Derivation |
|---|---|
| `activity_kind` ∈ `new_issue`, `new_pr`, `comment`, `review`, `state_change`, `ci`, `release`, `mention`, `assignment`, `security` | `reason` (`assign`, `mention`, `review_requested`, `ci_activity`, `state_change`, `security_alert`…) + `subject.type` + **"new item" test**: `latest_comment_url` empty or equal to `subject.url` ⇒ the item itself is new, not a reply; otherwise `comment` vs `review` from the latest comment URL shape / thread details. |
...
3. **Token permissions.** Setup guidance recommends a fine-grained PAT with read-only permissions; enabling `notifications` writes tells the user which extra permission the token needs. A 403 from the server is surfaced in Diagnostics, not retried.
...
- **M1 – MCP inbox, no AI:** accounts + keyring, `ghmcp` supervisor + client, `ghinbox mcp tools` to confirm tool names/shapes and record fixtures; notifications sync (`since`), `classify` activity kinds, `filter` rules, **Inbox groups** (New PRs / New issues / Replies…), **Mine** via search, mark read/done/snooze as local state (server `--read-only`; GitHub mirroring behind the per-account write setting, §4.9), tray badge. Useful on its own.
...
- `github-mcp-server` (v1.12.2 bundled): exact tool names, argument names (`since`, `filter`, `perPage`), JSON shapes in text content, diff size limits, confirm which tools the server classifies as write under `--read-only` (expected: `dismiss_notification`, `mark_all_notifications_read`, `manage_notification_subscription`, `manage_repository_notification_subscription`) so the read-only default still exposes everything the pipeline needs (run with toolsets `notifications,issues,pull_requests,repos`; §4.9 allowlist on top). Go `tool` directive build across arm64.
